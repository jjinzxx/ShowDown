...
| 멀티 베팅/공개/총격 사이 템포 | `BP_ShowDownModeBase` | `MultiplayerBetActionIntervalSeconds`, `MultiplayerPreRevealDelaySeconds`, `MultiplayerRevealToLoserSpotlightSeconds`, `MultiplayerLoserSpotlightHoldSeconds`, `MultiplayerPostShotPauseSeconds` |
...
- `MultiplayerPreRevealDelaySeconds`: 전원 베팅 완료 후 암흑이 짙어지고 카드 공개까지 대기. 최소 `2.0초`
- `MultiplayerRevealToLoserSpotlightSeconds`: 공개 카드가 모두 정착한 뒤 패자 적색 스포트라이트까지 대기. 기본값 `1.0초`
- `MultiplayerLoserSpotlightHoldSeconds`: 패자 적색 스포트라이트 이후 총 이동 시작까지 대기. 최소 `2.0초`
- `MultiplayerPostShotPauseSeconds`: 연속 총격 사이와 마지막 총격 후 다음 흐름 전 대기. 기본값 `0.5초`
- `bShowGameFlowDebugMessages`: 게임 흐름 디버그 메시지

멀티 흐름:
- 일반 베팅은 행동을 보여 준 뒤 `0.8초` 후 다음 차례로 넘어갑니다. 마지막 베팅에서는 암흑 전환을 시작하고 `2.0초` 후 공개합니다. 카드 정착 `1.0초` 후 패자를 붉게 비추고, 다시 `2.0초` 뒤 총 이동과 3인칭 카메라를 함께 시작합니다.
